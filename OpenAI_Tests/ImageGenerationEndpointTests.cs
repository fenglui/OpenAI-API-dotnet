...
namespace OpenAI_Tests
{
	public class ImageGenerationEndpointTests
	{
		...
		public void CreateImageWithUrl()
		{
			...
			var results = api.ImageGenerations.CreateImageAsync(new ImageGenerationRequest("A cyberpunk monkey hacker dreaming of a beautiful bunch of bananas, digital art", 1, ImageSize._512)).Result;
			Assert.IsNotNull(results);
			if (results.CreatedUnixTime.HasValue)
			{
				foreach (var image in results.Data)
				{
